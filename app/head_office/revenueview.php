                                        ...
                                        <tr>
                                            ...
                                            <td>
                                              <?php 
                                                ...
                                                $selecttechvalue = "SELECT SUM(a.service_charges + a.totalpartscost + a.total_charges - a.less_deposit - a.less_discount ) as total FROM jb_cost a, jb_joborder b WHERE b.isdeleted = 0 AND b.jobclear = 0 AND a.jobid = b.jobid AND b.repair_status <> 'Ready for Delivery' AND b.repair_status <> 'Ready for Delivery' AND b.repair_status <> 'Waiting for SOA Approval' AND b.repair_status <> 'Waiting List'  AND b.branchid  = '".$_GET['id']."' AND YEAR(b.created_at) = '".$currtYear."' ".$range."";
