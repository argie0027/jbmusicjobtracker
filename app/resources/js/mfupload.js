...
(function( $ ){
	...
	if(opt) $.extend(settings, opt);
	  ...
	  if ( !((window.location != window.parent.location) ? true : false) )
	  {
		...
		$(this).append('<div class="mf_upload_m"><form method="post" enctype="multipart/form-data" class="file" action="'+settings.post_upload+'" target="'+settings.iframe+'"><input type="file" class="file" name="'+settings.file_elem+'" id="'+settings.file_elem+'" single accept="image/jpeg,image/jpg,image/png" /><input type="hidden" name="folder" value="'+settings.folder+'" /><input type="hidden" name="file_element" value="'+settings.file_elem+'" /></form><div class="mf_upload_ins"></div></div>');
