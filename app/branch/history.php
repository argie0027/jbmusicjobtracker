        ...
        <section class="sidebar">
			...
			<?php 
			    ...
			    if(isset($_GET['daterange'])){
                    ...
                    $qu = "SELECT * FROM `jb_history` WHERE isbranch = ".$_SESSION['Branchid']." AND created_at BETWEEN '".$bydate[0]."' AND '".$bydate[1]."' ORDER BY created_at DESC";
                    $query = $db->ReadData($qu);
                    sidebarMenu($db->GetNumberOfRows());
                    $headertitle = "Job Order";
			    }else{

		    		$qu = "SELECT * FROM `jb_history` WHERE isbranch = ".$_SESSION['Branchid']." ORDER BY created_at DESC";
					$query = $db->ReadData($qu);
					sidebarMenu($db->GetNumberOfRows());
					$headertitle = "Job Order";
			     }
                    ...
                    <?php 
                        ...
                        if(isset($_GET['daterange'])){
                            ?>
                                ...
                                if ( filter.length ) {
                                    var query = "SELECT * FROM `jb_history` WHERE ( jobnumber LIKE '%"+filter+"%' OR description LIKE '%"+filter+"%' OR branch LIKE '%"+filter+"%' OR name LIKE '%"+filter+"%' ) AND isbranch = <?php echo $_SESSION['Branchid'] ?> AND created_at BETWEEN '"+daterange[0]+"' AND '"+daterange[1]+"' ORDER BY created_at DESC";
                                } else {
                            ...
                            ?>
                                var filter = $('#example1_filter label input').val();
                                if ( filter.length ) {
                                    var query = "SELECT * FROM `jb_history` WHERE ( jobnumber LIKE '%"+filter+"%' OR description LIKE '%"+filter+"%' OR branch LIKE '%"+filter+"%' OR name LIKE '%"+filter+"%' ) AND isbranch = <?php echo $_SESSION['Branchid'] ?> ORDER BY created_at DESC";
                                } else {
                                    var query = "<?php echo $queryforexcel; ?>";
                                }
            ...
            function(start, end) {
                    ...
                    <?php 
                        if(isset($_GET['daterange'])){
                            ...
                            if(isset($_GET['type'])){
                                ?>
                                window.location.assign("" + "<?php echo SITE_URL;?>branch/history.php?type=<?php echo $_GET['type']; ?>" + "&daterange=" + start.format('YYYY-MM-D 00:00:00') + 'to' + end.format('YYYY-MM-D 23:59:59') );
                                <?php 
                            }else{
                                ?>
                                window.location.assign("" + "<?php echo SITE_URL;?>branch/history.php?" + "&daterange=" + start.format('YYYY-MM-D 00:00:00') + 'to' + end.format('YYYY-MM-D 23:59:59'));
