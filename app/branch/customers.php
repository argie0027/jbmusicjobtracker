     ...
     <img src="<?php echo SITE_IMAGES_DIR; ?>ajax.gif">
                                        ...
                                        <tbody>
                                            ...
                                            if(isset($_GET['daterange'])){
                                                $bydate = split ("to", $_GET['daterange']);
                                                 ...
                                                 $sql = "SELECT * FROM `jb_customer` WHERE branchid = '".$_SESSION['Branchid']."' AND created_at BETWEEN '".$bydate[0]."' AND '".$bydate[1]."' ORDER BY created_at DESC";

                                            }else{
                                                $sql = "SELECT * FROM `jb_customer` WHERE branchid = '".$_SESSION['Branchid']."' ORDER BY created_at ASC";
                                            }
                                            $queryforexcel = $sql;
        ...
        <div class="modal fade" id="delete-modal" tabindex="-1" role="dialog" aria-hidden="true">
            ...
            <div class="modal-header">
                ...
                <h4 class="modal-title" ><i class="fa  fa-times-circle"> </i> Are you sure you want to delete this <span id="customername"></span> as Customer?</h4>
            </div>
                ...
                <section class="content invoice">                    
                    ...
                    <div class="row no-print">
                        <div class="col-xs-12">
                         ...
                         <!-- <button type="submit" class="btn btn-success pull-left "><i class="fa fa-plus"></i> OK </button> -->
                    </div><!-- /.modal-content --> 
                    </div><!-- /.modal-dialog -->
                    </div><!-- /.modal -->
                </div><!-- /.modal -->
        ...
        <script type="text/javascript">
            ...
            $('#createexcel').on('click', function(){
                <?php if(isset($_GET['daterange'])) { ?>
                    ...
                    if ( filter.length ) {
                        var query = "SELECT * FROM `jb_customer` WHERE ( name LIKE '%"+filter+"%' OR number LIKE '%"+filter+"%' OR email LIKE '%"+filter+"%') AND branchid = <?php echo $_SESSION['Branchid'] ?> AND created_at BETWEEN '"+daterange[0]+"' AND '"+daterange[1]+"' ORDER BY created_at DESC";
                    } else {
                    ...
                    var page = '../ajax/generateexcelbranch.php?querytogenerate='+query+"&&type=branchcustomer&&filename=branchcustomerexcel";
                ...
                <?php } else { ?>
                    ...
                    if ( filter.length ) {
                        var query = "SELECT * FROM `jb_customer` WHERE ( name LIKE '%"+filter+"%' OR number LIKE '%"+filter+"%' OR email LIKE '%"+filter+"%') AND branchid = <?php echo $_SESSION['Branchid'] ?> ORDER BY created_at DESC";
                    } else {
                        var query = "<?php echo $queryforexcel; ?>";
                    }
            ...
            function(start, end) {
                    ...
                    <?php 
                        if(isset($_GET['daterange'])){
                            ...
                            if(isset($_GET['type'])){
                                ?>
                                window.location.assign("" + "<?php echo SITE_URL;?>branch/customers.php?type=<?php echo $_GET['type']; ?>" + "&daterange=" + start.format('YYYY-MM-D 00:00:00') + 'to' + end.format('YYYY-MM-D 23:59:59') );
                                <?php 
                            }else{
                                ?>
                                window.location.assign("" + "<?php echo SITE_URL;?>branch/customers.php?" + "&daterange=" + start.format('YYYY-MM-D 00:00:00') + 'to' + end.format('YYYY-MM-D 23:59:59') );
                ...
                $('.delete').on('click',function(){
                    if(ID) {
                         ...
                         $("#customername").html($('#example1 #'+ID+' td:nth-child(2)').text());
                    }else {
                        $("#selecrecord-modal").modal("show");
                    }
                });
